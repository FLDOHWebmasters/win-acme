...
namespace PKISharp.WACS.Plugins.Base.Factories
{
    ...
    {
        async Task<InstallationPluginOptions> IInstallationPluginOptionsFactory.Acquire(Target target, IInputService inputService, RunLevel runLevel) => await Acquire(target, inputService, runLevel);
        async Task<InstallationPluginOptions> IInstallationPluginOptionsFactory.Default(Target target) => await Default(target);
        public abstract Task<TOptions> Acquire(Target target, IInputService inputService, RunLevel runLevel);
